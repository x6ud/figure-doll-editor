    ...
    begin(ctx: EditorContext) {
        ...
        if (!this.dragging) {
            ...
            if (this.detMat.length) {
                _tmp.copy(this.nodes[0].getWorldMatrix()).invert();
            }
            for (let i = 0; i < this.detMat.length; ++i) {
                if (!this.detMat[i]) {
                    this.detMat[i] = new Matrix4();
                }
                this.detMat[i].multiplyMatrices(_tmp, this.nodes[i + 1].getWorldMatrix());
            }
        }
        this.dragging = false;
    }
    ...
    update(ctx: EditorContext, view: EditorView): void {
        ...
        if (control.dragging) {
            ...
            for (let i = 0; i < this.nodes.length; ++i) {
                ...
                _tmp.copy(node.getParentWorldMatrix()).invert().multiply(matrix);
                if (i > 0) {
                    _tmp.multiply(this.detMat[i - 1]);
                }
                ctx.history.setValue(node, CPosition, getTranslation(new Vector3(), _tmp));
            }
        }
    }
