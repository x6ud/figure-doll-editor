...
// Modified from https://github.com/stephomi/sculptgl/blob/master/src/editing/tools/Move.js
export default class SculptMoveTool extends EditorTool {
    label = 'Sculpt Move';
    icon = icon;
    sculpt = true;
    brushStrength = 1;
    ...
    update(ctx: EditorContext, view: EditorView) {
        ...
        if (ctx.sculptStartThisFrame) {
            this.nodeId = node.id;
            this.stroke0 = this.sculptPickStrokeVertices(ctx, node, view, mesh, this.frontFacesOnly);
            if (!this.stroke0.track.length) {
                this.nodeId = 0;
        ...
        if (!this.stroke0 || !this.stroke0.track.length) {
            return;
        }
